...
def render_setup_page():
    ...
    with col1:
        st.markdown('<div class="section-header"><span class="stepper-badge">1</span>Select Assets</div>', unsafe_allow_html=True)
        selection_mode = st.radio("Selection mode", ["Search / Manual", "By Sector"], horizontal=True, key="selection_mode")

        if selection_mode == "By Sector":
            sectors = sorted(stock_db["sector"].unique())
            chosen_sector = st.selectbox("Sector", sectors, key="chosen_sector")
            sector_stocks = stock_db[stock_db["sector"] == chosen_sector]
            options = [f"{r.symbol} — {r.company_name}" for r in sector_stocks.itertuples()]
            default_picks = [o for o in options if o.split(" — ")[0] in st.session_state.selected_symbols]
            picked = st.multiselect("Companies in this sector", options, default=default_picks, key="sector_picks")
            picked_symbols = [p.split(" — ")[0] for p in picked]
        else:
            search_term = st.text_input("Search by name or ticker", "", key="search_term")
            if search_term:
                mask = stock_db["symbol"].str.contains(search_term, case=False) | stock_db["company_name"].str.contains(search_term, case=False)
                filtered = stock_db[mask]
            else:
                filtered = stock_db
            options = [f"{r.symbol} — {r.company_name}" for r in filtered.itertuples()]
            default_picks = [
                f"{s} — {symbol_to_name[s]}" for s in st.session_state.selected_symbols if s in symbol_to_name
            ]
            picked = st.multiselect("Select stocks (2–25)", options, default=default_picks, key="manual_picks")
            picked_symbols = [p.split(" — ")[0] for p in picked]

        picked_symbols = list(dict.fromkeys(picked_symbols))
        if picked_symbols:
            st.session_state.selected_symbols = picked_symbols
        st.caption(f"Selected: {len(picked_symbols)} stock(s). Choose 2–25. Dataset covers {DATASET_MIN} to {DATASET_MAX}.")

        with st.expander("Browse full stock list"):
            st.dataframe(stock_db, use_container_width=True, height=300)

    with col2:
        st.markdown('<div class="section-header"><span class="stepper-badge">2</span>Investment Settings</div>', unsafe_allow_html=True)
        investment_amount = st.number_input("Total Investment (PKR)", min_value=1000, value=150000, step=5000, key="investment_amount")

        default_end = DATASET_MAX
        default_start = max(DATASET_MIN, date(default_end.year - 3, default_end.month, default_end.day))
        c_a, c_b = st.columns(2)
        start_date = c_a.date_input("Start Date", value=default_start, min_value=DATASET_MIN, max_value=DATASET_MAX, key="start_date")
        end_date = c_b.date_input("End Date", value=default_end, min_value=DATASET_MIN, max_value=DATASET_MAX, key="end_date")

        risk_free_rate = st.slider("Risk-Free Rate (annual)", 0.0, 0.30, RISK_FREE_DEFAULT, 0.005, format="%.3f", key="risk_free_rate")

        st.markdown('<div class="section-header"><span class="stepper-badge">3</span>Strategy</div>', unsafe_allow_html=True)
        strategy_mode = st.radio(
            "Optimization Strategy",
            ["Maximum Sharpe Ratio", "Minimum Volatility", "Compare Both"],
            key="strategy_mode",
        )

        if strategy_mode == "Maximum Sharpe Ratio":
            st.markdown(
                '<div class="info-box">Seeks the highest estimated <b>risk-adjusted</b> return — balances '
                'expected return against volatility rather than simply maximizing return.</div>', unsafe_allow_html=True)
        elif strategy_mode == "Minimum Volatility":
            st.markdown(
                '<div class="info-box">Seeks the portfolio with the <b>lowest historical volatility</b> '
                'among the selected assets, regardless of expected return.</div>', unsafe_allow_html=True)
        else:
            st.markdown(
                '<div class="info-box">Calculates <b>both</b> strategies so you can compare them side by side '
                'on the dashboard, and switch between them freely.</div>', unsafe_allow_html=True)

    st.markdown("---")
    _, mid, _ = st.columns([1, 1, 1])
    with mid:
        run_button = st.button("🚀 Optimize Portfolio", use_container_width=True, type="primary")

    if run_button:
        result = run_pipeline(picked_symbols, investment_amount, start_date, end_date, risk_free_rate, strategy_mode)
        if result:
            st.session_state.results = result
            st.session_state.active_view_strategy = "Maximum Sharpe Ratio" if strategy_mode != "Minimum Volatility" else "Minimum Volatility"
    ...
    st.markdown(
        ...
        'market data and mathematical models. Historical performance does not guarantee future results.</div>',
        unsafe_allow_html=True,
    )
